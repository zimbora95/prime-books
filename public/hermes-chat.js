...
(function () {
  ...
  async function readingBlock() {
    ...
    if (!R || !R.ready) {
      /* Workshop shell (no PDF): say so instead of an empty block. */
      if (R && R.meta && R.meta.title && !R.meta.pdf) {
        return [
          "[READING CONTEXT - generated by the app, always trust this]",
          "Book: " + (R.meta.title || "Prime Books") + (R.meta.band ? " (" + R.meta.band + ")" : ""),
          "This book has NO PDF yet - it exists only as a catalogue entry. No pages are open and none can be.",
          "The teacher wants to CREATE this book. You can build it from scratch: generate pages with pymupdf,",
          "illustrations with tools/pb_image_gen.py (gpt-image-2), create public/library/<slug>/ with book.pdf + cover.webp,",
          "and add the row to public/library.json. Work unit by unit; propose an outline first.",
          "[END READING CONTEXT]",
        ].join("\n");
      }
      return "";
  ...
  async function send(question) {
    ...
    var R = window.PBReading;
    if (!R) {
      setStatus("The reader is not ready yet.", false);
      return;
    }
    /* A book with no PDF yet (workshop shell) is still fully chatable:
       meta exists, pages are just zero. */
    if (!R.ready && !(R.meta && R.meta.title && !R.meta.pdf)) {
      ...
      for (;;) {
        ...
        for (var i = 0; i < frames.length; i++) {
          ...
          if (ev.event === "assistant.delta" && ev.data && ev.data.delta) {
            ...
            /* If the agent says it changed the book, offer a one-click reload
               so the teacher sees the new PDF without hunting for F5. */
            var done = (ev.data.content || acc || "");
            if (/\b(saved|changed|updated|edited|regenerated|rebuilt|done)\b/i.test(done) &&
                /\b(pdf|cover|page|book|title|exercise)\b/i.test(done) &&
                state.meta && state.meta.pdf) {
              var wrap = h("div", "pbc-reload");
              var rb = h("button", "pbc-reloadbtn", "\u21bb See the updated book");
              rb.type = "button";
              rb.addEventListener("click", function () {
                if (window.PBLibrary && typeof window.PBLibrary.reload === "function")
                  window.PBLibrary.reload();
  ...
  function mount() {
    ...
    /* Composer: a single auto-growing textarea with the attach button
       INSIDE the form, below-left of the input row. Layout:
         [ textarea ................................. ]
         [ (attachments chips row when present)      ]
         [ (paperclip) (send ->)              toolbar ]
    */
    var form = h("form", "pbc-form");
    var composer = h("div", "pbc-composer");
    el.input = h("textarea", "pbc-input");
    el.input.rows = 1;
    el.input.placeholder = "Ask me to edit this book, or ask about the pages\u2026";
    el.input.setAttribute("aria-label", "Message the book workshop");
    composer.appendChild(el.input);

    el.attachRow = h("div", "pbc-attach");
    el.attachRow.style.display = "none";
    composer.appendChild(el.attachRow);

    var toolbar = h("div", "pbc-toolbar");
    ...
    el.send.type = "submit";
    toolbar.appendChild(el.attachBtn);
    toolbar.appendChild(el.fileInput);
    toolbar.appendChild(el.send);
